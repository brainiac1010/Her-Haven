...
const PostAReview = ({ isModalOpen, handleClose }) => {
    ...
    return (
        <div className={`fixed inset-0 bg-black/90 flex items-center justify-center z-40 px-2 ${isModalOpen ? '' : 'hidden'}`}>
            <div className="bg-white p-6 rounded-md shadow-lg w-96 z-50">
                ...
                <textarea
                    ...
                    rows="4"
                    className="focus:outline-none w-full border border-gray-300 p-2 rounded-md mb-4"
                ></textarea>
                <div className="flex justify-end gap-2">
                    <button onClick={handleClose} className="px-4 py-2 bg-gray-300 rounded-md">
                        Cancel
                    </button>
                    <button onClick={handelSubmit} className="px-4 py-2 bg-primary text-white rounded-md">
                        Submit
                    </button>
                </div>
            </div>
